...
- Setup file: `src/test/setup.ts` (globals, mocks, cleanup)
- Test files: `**/*.{test,spec}.{ts,tsx}` in `src/`

**Running a single test file:**
```bash
npx vitest run path/to/test.spec.ts
npx vitest watch path/to/test.spec.ts  # watch mode
```

**Existing Tests:**
- `src/features/emulator/useEmulatorStore.test.ts` - Load functionality state management (14 tests)
- `src/features/control/ControlBar.test.tsx` - UI state and interactions (18 tests)
...
- Run tests (`npm test` or `npm run test:run`) to ensure correctness
